...
const formattedChunks = rawData.map(item => {
...
const chunkPath = path.join(__dirname, 'chunks', '018_THOI_DIEM_DU_LICH_63_TINH.json');
fs.writeFileSync(chunkPath, JSON.stringify(formattedChunks, null, 2), 'utf-8');
console.log(`Saved ${formattedChunks.length} chunks to:`, chunkPath);

const backendDataDir = path.join(__dirname, '..', 'backend', 'src', 'data');
if (!fs.existsSync(backendDataDir)) {
  fs.mkdirSync(backendDataDir, { recursive: true });
}
const backendDataPath = path.join(backendDataDir, 'travel_seasons_63_provinces.json');
fs.writeFileSync(backendDataPath, JSON.stringify(rawData, null, 2), 'utf-8');
console.log(`Saved backend dataset to:`, backendDataPath);
