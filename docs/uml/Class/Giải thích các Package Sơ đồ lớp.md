...
| Class | Ý nghĩa |
| :--- | :--- |
| **AuthRouter** | Định tuyến các yêu cầu HTTP liên quan đến đăng ký, đăng nhập, xác thực và quên mật khẩu. |
| **AuthMiddleware** | Middleware kiểm tra tính hợp lệ của Token JWT và phân quyền vai trò (User/Admin). |
| **EmailService** | Dịch vụ gửi email xác thực tài khoản và email khôi phục mật khẩu. |
| **AuthRequest** | Giao diện định nghĩa thông tin người dùng được giải mã từ JWT đính kèm vào HTTP Request. |
...
| Class | Ý nghĩa |
| :--- | :--- |
| **TripsRouter** | Tiếp nhận và điều hướng các yêu cầu tạo, sửa, xóa, sao chép và tối ưu chuyến đi. |
| **CreateTripDto** | Định nghĩa cấu trúc dữ liệu bắt buộc khi người dùng tạo mới một hành trình. |
| **UpdateTripDto** | Định nghĩa cấu trúc các trường thông tin cho phép cập nhật của chuyến đi. |
...
| Class | Ý nghĩa |
| :--- | :--- |
| **PostsRouter** | Định tuyến yêu cầu lấy danh sách bài đăng, bình luận và cập nhật tương tác của người dùng. |
...
| Class | Ý nghĩa |
| :--- | :--- |
| **MapRouter** | Định tuyến yêu cầu tìm kiếm, định vị và geocoding địa lý. |
...
# Package: Chatbot & Dialogue Layer
...
| Class | Ý nghĩa |
| :--- | :--- |
| **ChatbotController** | Tiếp nhận yêu cầu trò chuyện và gửi phản hồi của trợ lý ảo. |
| **ChatbotService** | Điều phối luồng xử lý tin nhắn, lưu trữ lịch sử và gọi RAG. |
| **ChatbotRepository** | Ghi nhận lịch sử hội thoại thực tế của người dùng với trợ lý ảo vào database. |
| **ConversationStateService**| Quản lý và cập nhật trạng thái hiện tại của phiên trò chuyện. |
| **ConversationIntelligence**| Bộ não điều phối các chính sách khớp thông tin và phân tích ngữ nghĩa hội thoại. |
| **EmotionAnalyzer** | Phân tích trạng thái cảm xúc của người dùng (vui vẻ, chán nản, thất vọng). |
| **ConversationStateMachine**| Máy trạng thái quản lý chu trình hội thoại của bot. |
| **SlotPolicy** | Quyết định chiến lược điền thông tin chuyến đi còn thiếu. |
| **ResponsePolicy** | Xác định phong cách và giọng điệu phản hồi phù hợp với cảm xúc người dùng. |
| **RuleOverrideEngine** | Bộ luật đè ưu tiên xử lý các trường hợp hội thoại đặc biệt hoặc khẩn cấp. |
| **TravelIntentService** | Phân tích ý định du lịch của người dùng. |
| **TravelRerankerService** | Sắp xếp lại thứ tự các địa điểm gợi ý theo sở thích cá nhân. |
| **SuggestionBuilderService**| Tự động tạo các câu gợi ý nhanh hiển thị trên UI. |
| **SlotFillingService** | Trích xuất thông tin ngày đi, điểm đến từ văn bản thô. |
| **ResponsePlanner** | Lập kế hoạch phân bổ nội dung phản hồi của bot. |
| **ResponseFormatterService**| Định dạng câu trả lời rõ ràng, sinh động. |
| **PromptComposerService** | Soạn thảo Prompt động gửi lên mô hình ngôn ngữ lớn (LLM). |
...
ChatbotController
 └── ChatbotService
      ...
      └── ConversationIntelligence
           ...
           ├── SlotPolicy
           └── Dialogue Layer Services (Intent, Slot, Prompt, v.v.)
```
...
| **BaseTool** | Lớp cơ sở định nghĩa cấu trúc của một công cụ hỗ trợ AI (Agent Tool). |
| **MapTool** / **WeatherTool** | Công cụ tra cứu bản đồ địa điểm và thông tin thời tiết phục vụ lập lịch. |
| **FoodTool** / **CultureTool** | Công cụ tra cứu ẩm thực bản địa và lịch sử văn hóa vùng miền. |
| **RecommendationTool** | Công cụ truy vấn sở thích người dùng để Agent cá nhân hóa đề xuất. |
| **ItineraryTool** | Công cụ hỗ trợ Agent tạo trực tiếp các hoạt động chuyến đi vào DB. |
...
| **EmbeddingsService** | Dịch vụ gọi mô hình Vector hóa văn bản thô. |
| **VectorStoreService** | Tương tác với CSDL Vector (PostgreSQL pgvector) để tìm tài liệu tương đồng. |
| **RetrieverService** | Thực hiện lọc và xếp hạng các tài liệu liên quan nhất. |
| **PromptBuilderService** | Kết hợp câu hỏi của người dùng và ngữ cảnh tri thức thành Prompt hoàn chỉnh. |
| **SemanticCacheService** | Cache ngữ nghĩa để phản hồi ngay các câu hỏi trùng lặp mà không cần gọi LLM. |
| **GuardrailsService** | Hệ thống rào chắn bảo vệ ngăn chặn Prompt Injection và ngôn từ không phù hợp. |
| **FactVerifierService** | Đối chiếu câu trả lời sinh ra với ngữ cảnh tri thức để kiểm chứng sự thật. |
| **CitationGeneratorService**| Tạo các liên kết trích nguồn trực quan đính kèm vào câu trả lời của AI. |
| **KnowledgeRepository** | Kho lưu trữ thông tin tri thức thô trong CSDL. |
| **RagAuditRepository** | Ghi nhật ký kiểm toán hoạt động của RAG phục vụ giám sát hiệu năng. |
...
| Class | Ý nghĩa |
| :--- | :--- |
| **FeedbackController** | Tiếp nhận yêu cầu gửi và thống kê phản hồi của người dùng. |
| **FeedbackService** | Xử lý logic ghi nhận và phân tích dữ liệu thống kê. |
| **FeedbackRepository** | Ghi nhận thông tin phản hồi vào cơ sở dữ liệu. |
...
## Vai trò trong hệ thống
Hỗ trợ quản trị viên thu thập dữ liệu trải nghiệm người dùng thực tế.
...
| Class | Ý nghĩa |
| :--- | :--- |
| **CacheController** | Cung cấp các công cụ quản trị cache (xem danh sách khóa, xóa cache). |
| **CacheService** | Thực thi logic đọc, lưu và kiểm tra tính hợp lệ của cache. |
| **CacheRepository** | Kết nối trực tiếp tới Redis hoặc vùng nhớ cache vật lý của hệ thống. |
...
| Class | Ý nghĩa |
| :--- | :--- |
| **TravelHistoryController**| Tiếp nhận yêu cầu quản lý lịch sử du lịch của người dùng. |
| **TravelHistoryService** | Thực thi nghiệp vụ kiểm tra và cập nhật vết lịch sử. |
| **TravelHistoryRepository**| Truy vấn và ghi nhận dữ liệu lịch sử vào database. |
...
| Class | Ý nghĩa |
| :--- | :--- |
| **ItineraryController** | Định tuyến yêu cầu tạo, sửa, xóa nhật ký hành trình. |
| **ItineraryService** | Điều phối logic cập nhật nhật ký và tọa độ các tuyến đường. |
| **ItineraryRepository** | Ghi nhận dữ liệu hành trình vào cơ sở dữ liệu. |
...
| Class | Ý nghĩa |
| :--- | :--- |
| **ToolCallController** | Cung cấp các API truy xuất vết cuộc gọi công cụ. |
| **ToolCallService** | Thực thi nghiệp vụ kiểm tra tính hợp lệ của cuộc gọi. |
| **ToolCallRepository** | Lưu vết cuộc gọi vào CSDL. |
...
| Class | Ý nghĩa |
| :--- | :--- |
| **User** | Thực thể đại diện cho tài khoản đăng nhập chính của người dùng. |
| **Profile** | Thực thể lưu thông tin cá nhân mở rộng (ảnh đại diện, tên, bio). |
| **TravelPreferences** | Thực thể lưu trữ các sở thích du lịch tĩnh do người dùng tự khai báo. |
| **AIMemory** | Thực thể lưu trữ bộ nhớ động do AI tự động đúc rút qua các cuộc hội thoại. |
| **Follower** | Thực thể biểu diễn quan hệ nhiều-nhiều tự tham chiếu của người theo dõi. |
| **Notification** | Thực thể lưu trữ các thông báo gửi đến người dùng. |
...
| Class | Ý nghĩa |
| :--- | :--- |
| **Trip** | Thực thể đại diện cho một chuyến đi cụ thể của người dùng. |
| **TripDay** | Thực thể đại diện cho một ngày cụ thể nằm trong chuyến đi. |
| **TripActivity** | Thực thể chi tiết một hoạt động cụ thể (tham quan, ăn uống, di chuyển). |
| **Destination** | Thực thể đại diện cho một địa danh/địa điểm có trên bản đồ hệ thống. |
| **SavedPlace** | Thực thể lưu trữ các địa điểm do người dùng tự đánh dấu lưu lại. |
| **FavoriteFood** | Thực thể lưu các món ăn yêu thích do người dùng tự lưu giữ. |
...
| Class | Ý nghĩa |
| :--- | :--- |
| **Post** | Thực thể đại diện cho một bài viết được đăng tải trên Bảng tin. |
| **Comment** | Thực thể lưu trữ bình luận dưới bài viết (hỗ trợ bình luận phân cấp parentId). |
| **Like** | Thực thể lưu vết tương tác thích bài viết. |
| **Bookmark** | Thực thể lưu vết đánh dấu bài viết của người dùng. |
...
| Tên Package | Phân nhóm Chức năng | Vai trò trong hệ thống |
| :--- | :--- | :--- |
**| **Authentication Module** | Bảo mật hệ thống | Xác thực tài khoản, phân quyền truy cập API bằng JWT. |
**| **Trips Module** | Lập lịch du lịch | Xử lý nghiệp vụ tạo, sửa, xóa hành trình và tối ưu hóa tuyến đường. |
**| **Posts Module** | Mạng xã hội du lịch | Xử lý các hoạt động đăng bài, bình luận, tương tác cộng đồng. |
**| **Map Module** | Bản đồ không gian | Dịch vụ tìm kiếm địa danh, geocoding và hiển thị bản đồ Leaflet. |
**| **Recommendations Module** | Cá nhân hóa | Sinh đề xuất thông minh dựa trên sở thích tĩnh của người dùng. |
**| **Chatbot & Dialogue Layer** | Trí tuệ hội thoại | Bộ não thấu hiểu cảm xúc, phân tích ý định và soạn thảo Prompt. |
**| **AI Agents Module** | Tác vụ tự động | Thực thi kế hoạch đa bước và kích hoạt công cụ (Tools) của AI. |
...
**| **User Entities** | Mô hình CSDL | Định nghĩa các bảng dữ liệu về tài khoản, hồ sơ cá nhân. |
**| **Trip Entities** | Mô hình CSDL | Định nghĩa các bảng dữ liệu về chuyến đi, hoạt động và địa điểm. |
**| **Post Entities** | Mô hình CSDL | Định nghĩa các bảng dữ liệu về bài viết, tương tác thích/lưu. |
