...
| Class | Ý nghĩa |
| :--- | :--- |
| **Xu Ly Yeu Cau Chatbot (ChatbotController)** | Tiếp nhận yêu cầu trò chuyện và gửi phản hồi của trợ lý ảo. |
| **Dich Vu Xu Ly Chatbot (ChatbotService)** | Điều phối luồng xử lý tin nhắn, lưu trữ lịch sử và gọi RAG. |
| **Luu Tru Chatbot (ChatbotRepository)** | Ghi nhận lịch sử hội thoại thực tế của người dùng với trợ lý ảo vào database. |
| **Luu Trang Thai Chat (ConversationStateService)** | Quản lý và cập nhật trạng thái hiện tại của phiên trò chuyện. |
| **Bo Nao Chatbot (ConversationIntelligence)** | Bộ xử lý trung tâm (CIM) phân tích cấu trúc câu hỏi, điều phối các module cảm xúc, trạng thái và luật trả lời. |
| **Doc Cam Xuc Khach Hang (EmotionAnalyzer)** | Phân tích trạng thái cảm xúc của người dùng (vui vẻ, chán nản, thất vọng). |
| **Quan Ly Buoc Hoi Thoai (ConversationStateMachine)** | Máy trạng thái quản lý chu trình hội thoại của bot. |
| **Luat Dien Thong Tin (SlotPolicy)** | Quyết định chiến lược điền thông tin chuyến đi còn thiếu. |
| **Luat Tra Loi (ResponsePolicy)** | Xác định phong cách và giọng điệu phản hồi phù hợp với cảm xúc người dùng. |
| **Bo Sua Loi Chatbot (RuleOverrideEngine)** | Bộ luật đè ưu tiên xử lý các trường hợp hội thoại đặc biệt hoặc khẩn cấp. |
| **Doan Y Dinh Nguoi Dung (TravelIntentService)** | Phân tích ý định du lịch của người dùng. |
| **Chon Loc Dia Diem Hot (TravelRerankerService)** | Sắp xếp lại thứ tự các địa điểm gợi ý theo sở thích cá nhân. |
| **Tao Cau Goi Y (SuggestionBuilderService)** | Tự động tạo các câu gợi ý nhanh hiển thị trên UI. |
| **Ghi Nhan Thong Tin Lich Trinh (SlotFillingService)** | Trích xuất thông tin ngày đi, điểm đến từ văn bản thô. |
| **Len Ke Hoach Cau Tra Loi (ResponsePlanner)** | Lập kế hoạch phân bổ nội dung phản hồi của bot. |
| **Trinh Bay Cau Tra Loi (ResponseFormatterService)** | Định dạng câu trả lời rõ ràng, sinh động. |
| **Ghep Thong Tin Gui AI (PromptComposerService)** | Soạn thảo Prompt động gửi lên mô hình ngôn ngữ lớn (LLM). |
