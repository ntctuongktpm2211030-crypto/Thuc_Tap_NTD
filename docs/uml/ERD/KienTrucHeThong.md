...
## 2. PHÂN TÍCH CHI TIẾT 17 MODULES CỦA HỆ THỐNG
Hệ thống Backend được phân mảnh thành đúng **17 module nghiệp vụ và hạ tầng** được đăng ký làm các Express Routers trong `backend/src/app.ts`. Dưới đây là bảng phân tích toàn diện:
...
| **9** | **itinerary** | `routes/itinerary.router.ts` | `controllers/itinerary.controller.ts` | `services/itinerary.service.ts` | `repositories/itinerary.repository.ts` | `middlewares/itinerary.validation.ts` | `Trip`, `TripDay`, `TripActivity` | Quản lý lịch trình du lịch đã sinh, chia ngày đi chi tiết và điều phối các hoạt động cụ thể. |
...
| **14** | **cache** | `routes/cache.router.ts` | `controllers/cache.controller.ts` | `services/cache.service.ts` | `repositories/cache.repository.ts` | Cache Cleanup Job trong `server.ts` (6 tiếng/lần) | `SystemCache`, `CacheMetadata` | Lớp đệm cache dữ liệu hoặc các truy vấn nặng có cơ chế hết hạn (TTL-based). |
...
| **17** | **travel-history** | `routes/travel-history.router.ts` | `controllers/travel-history.controller.ts` | `services/travel-history.service.ts` | `repositories/travel-history.repository.ts` | `middlewares/travel-history.validation.ts` | `TravelHistory` | Lưu vết lịch sử các chuyến du lịch thực tế người dùng đã đi (thời gian, chi phí, địa điểm) nhằm làm giàu dữ liệu gợi ý. |

*Lưu ý: Các thành phần phụ trợ như thuật toán tối ưu hóa TSP (optimizer), quản lý ý định hội thoại (dialogue), hay geocoding (destinations) được tích hợp trực tiếp làm các hàm helper/thư viện nội bộ bên trong các module chính (`trips`, `chatbot`, `ai-agents`) chứ không cấu hình thành các API module độc lập.*
...
2. **requireAdmin** (trong [auth.middleware.ts](file:///d:/Thuc_Tap_NDT/backend/src/modules/auth/auth.middleware.ts)):
   ...
   - Thường sử dụng phía sau `requireAuth` để bảo vệ các API quản trị như quản lý Tri thức.
