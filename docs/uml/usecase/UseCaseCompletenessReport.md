...
| **Trip, Destination, Activity** | `/api/v1/trips/`, `/api/v1/itinerary/` | `UC_ManageTrips`, `UC_AITrip`, `UC_TSP`, `UC_Trip_Custom` | **Đạt** |
...
*   **Định tuyến Agent và RAG pipeline**: Đã được gom vào luồng xử lý nội bộ của Use Case `UC_Chatbot` và `UC_RAGSearch` vì đây là giải thuật tối ưu hóa, không phải mục tiêu hành vi của người dùng.
...
2.  **Xem và tạo sự kiện địa phương (`UC_LocalEvents`)**: Nghiệp vụ meetup tại điểm đến (`Event` trong module map).
3.  **Quản lý cuộc hội thoại (`UC_ManageChats`)**: Cho phép người dùng liệt kê, đổi tên hoặc xóa lịch sử các luồng chat trợ lý ảo.
4.  **Hành trình tự do (`UC_Trip_Custom`)**: Các nghiệp vụ CRUD Itinerary Days và Activities do người dùng thiết kế thủ công.
...
- **Tổng số tác nhân hệ thống hỗ trợ**: 6 (Firebase, OpenStreetMap, OpenAI, vietnamadminunits, SMTP, Supabase).
- **Tổng số Use Cases nghiệp vụ**: 28 Use Cases (Được phân bố hoàn hảo trên 5 phân hệ, không có module Admin).
