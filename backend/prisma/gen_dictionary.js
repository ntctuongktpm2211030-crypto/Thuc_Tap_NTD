...
let outMd = "# BẢNG TỪ ĐIỂN DỮ LIỆU CÁC BẢNG CỐT LÕI (DATABASE DICTIONARY)\n\n";
outMd += "Tài liệu từ điển dữ liệu chuẩn hóa cho 21 bảng cốt lõi của ứng dụng SmartTravel dựa trên file `schema.dbml` đã tinh giản.\n\n";
outMd += "Cấu trúc cột tuân thủ chính xác yêu cầu mẫu:\n";
outMd += "| STT | Thuộc tính | Kiểu dữ liệu | Null | Khóa | Mô tả |\n\n";
outMd += "---\n\n";
...
tables.forEach(t => {
    ...
    outMd += "| STT | Thuộc tính | Kiểu dữ liệu | Null | Khóa | Mô tả |\n";
    outMd += "| :--- | :--- | :--- | :---: | :---: | :--- |\n";
    t.cols.forEach((col, idx) => {
        const stt = idx + 1;
        const [name, dtype, nullable, key, desc] = col;
        outMd += `| ${stt} | ${name} | ${dtype} | ${nullable} | ${key} | ${desc} |\n`;
    });
    outMd += "\n---\n\n";
});

const filePath = path.join(__dirname, 'database_dictionary.md');
fs.writeFileSync(filePath, outMd, 'utf8');
console.log("SUCCESSFULLY_GENERATED_DATABASE_DICTIONARY_JS");
