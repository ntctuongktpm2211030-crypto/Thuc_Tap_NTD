...
router.get('/:id', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const trip = await db_1.default.trip.findUnique({
...
// ─────────────────────────────────────────────────────────
router.post('/optimize-route', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const { waypoints } = req.body;
        if (!waypoints || !Array.isArray(waypoints) || waypoints.length < 2) {
            return res.status(400).json({ error: 'At least 2 waypoints required.' });
        }
        const optimized = (0, route_optimizer_1.optimizeRoute)(waypoints);
...
router.put('/:id', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const existing = await db_1.default.trip.findUnique({ where: { id: req.params.id } });
        if (!existing)
            return res.status(404).json({ error: 'Trip not found.' });
...
router.delete('/:id', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        ...
        if (existing.ownerId !== req.user.sub)
            return res.status(403).json({ error: 'Access denied.' });
        await db_1.default.trip.delete({ where: { id: req.params.id } });
...
// ─────────────────────────────────────────────────────────
router.post('/:id/clone', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const source = await db_1.default.trip.findUnique({
            where: { id: req.params.id },
        ...
        const cloned = await db_1.default.trip.create({
            data: {
                ownerId: req.user.sub,
