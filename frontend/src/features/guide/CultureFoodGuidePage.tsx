      ...
      <div className="container-wide py-10">
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-bold transition-all duration-200 cursor-pointer ${
                  active 
                    ? 'bg-blue-600 text-white shadow-md shadow-blue-500/15 border-transparent hover:bg-blue-700 active:scale-98' 
                    : 'bg-white text-slate-700 border border-slate-200 hover:border-slate-300 hover:bg-slate-50 active:scale-98'
                }`}
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filtered.map(article => (
            <article key={article.id} className="bg-white border border-slate-200/60 rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">
              
              {/* Image & Badge */}
              <div className="aspect-[16/10] relative overflow-hidden bg-slate-100">
                <img src={article.image} alt={article.title} className="w-full h-full object-cover transition-transform duration-500" loading="lazy" />
                <span className={`absolute top-4 left-4 text-[9px] font-black px-2.5 py-1 rounded shadow-sm uppercase tracking-wider ${
              ...
              <div className="p-6 flex-1 flex flex-col justify-between">
                <div>
                  ...
                  {/* Title */}
                  <h2 className="text-slate-900 font-bold text-base leading-snug mb-2.5 hover:text-blue-600 transition-colors">
                    {article.title}
                  </h2>
                  
                  {/* Excerpt */}
                  <p className="text-slate-500 text-xs sm:text-sm leading-relaxed mb-5 line-clamp-3">
                    {article.excerpt}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-5">
                    {article.tags.map(tag => (
                      <span key={tag} className={`text-[10px] font-semibold px-2 py-0.5 rounded ${
                  ...
                  <div className="pt-4 border-t border-slate-100 flex items-center">
                    <span className="text-blue-600 font-bold text-xs hover:text-blue-700 transition-colors cursor-pointer inline-flex items-center gap-1 group">
                      {vi ? 'Đọc cẩm nang' : 'Read guide'} <ChevronRight size={13} className="stroke-[2.5px] transition-transform group-hover:translate-x-0.5" />
                    </span>
                  </div>
                </div>
